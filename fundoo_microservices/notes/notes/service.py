...
class NoteService(object):
    ...
    def read_label_service(self, request_data):
        ...
        if request_data.get('id') is not None:
            label = filter_by_id(table=Labels, id=request_data.get('id'))
    ...
    @rpc
    def update_label_service(self, request_data, id):
        """ this service  api  is used to update the label"""
        if session.query(Labels).filter_by(id=id).update({Labels.label:request_data.get('label')}):
            session.commit()
            session.close()
            return {'message': 'label updated successfully', 'data': request_data}
        else:
