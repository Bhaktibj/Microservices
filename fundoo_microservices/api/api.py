...
app = Flask(__name__)  # defined app or create app
Swagger(app)  # import app
...
def delete(pk):
    ...
    if token is None:  # check token is None or not
        ...
        with ClusterRpcProxy(HOST_CONFIG) as rpc:  # using cluster rpc
            result = rpc.noteService.delete_note_service(request_data)  # call delete note service api
